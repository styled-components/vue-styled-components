...
describe('with styles', () => {
	...
	it('should handle inline style objects with media queries', () => {
		...
		const vm = createApp(Comp).mount('body')
		expectCSSMatches(
			'.a {background-color: blue;}@media screen and (min-width: 250px) {.a {background-color: red;}}'
		)
	})
	...
	it('should handle inline style objects with pseudo selectors', () => {
		...
		const vm = createApp(Comp).mount('body')
		expectCSSMatches(
			'.a {background-color: blue;}.a:hover {-webkit-text-decoration: underline;text-decoration: underline;}'
		)
	})
	...
	it('should handle inline style objects with contextual selectors', () => {
		...
		const vm = createApp(Comp).mount('body')
		expectCSSMatches(
			'.a {background-color: blue;}html.something .a {color: white;}'
		)
	})
